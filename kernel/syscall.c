...
void syscall(void)
{
  ...
  if (num > 0 && num < NELEM(syscalls) && syscalls[num])
  {
    ...
    // Ví dụ: num=5 (SYS_read), 1 << 5 = 32.
    if (p->trace_mask && ((1 << num) & p->trace_mask)) {
      // Nếu có, in ra thông tin theo đúng định dạng
      // p->pid: ID của tiến trình
      // syscall_names[num]: Tên của system call
